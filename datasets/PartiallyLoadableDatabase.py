...
class PartiallyLoadableDatabase(Database, ABC):
    ...
    def build_shards(self, shard_size, skip=0):
        ...
        for i, (subset_id, images_shard, labels_shard) in enumerate(self.build_shards_iterator(shard_size, skip)):
            ...
            if subset_id in shards_dict:
                shards_dict[subset_id]["videos_filenames"] += [images_shard_filename]
                shards_dict[subset_id]["labels_filenames"] += [labels_shard_filename]
                shards_dict[subset_id]["samples_count"] += shard_size
            else:
                shards_dict[subset_id] = {"videos_filenames": [images_shard_filename],
    ...
    def normalize(self, target_min=0.0, target_max=1.0):
        ...
        self.train_subset.normalization_range = normalization_range
        self.test_subset.normalization_range = normalization_range
