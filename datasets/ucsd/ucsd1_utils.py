...
def removed_unlabeled_ucsd1_test_samples(database: UCSDDatabase):
    ...
    result = np.zeros([len(labeled) * video_length, *database.test_subset.images.shape[1:]], dtype=np.float32)
    for i, index in enumerate(labeled):
        result[i * 200: (i + 1) * video_length] = \
            database.test_subset.images[index * video_length: (index + 1) * video_length]
    database.test_subset.images = result
    database.test_subset.save_to_npz(force=True)
