...
def get_database(shift_brightness, dropout_noise):
    train_preprocessors = []
    if shift_brightness:
    ...
    ucsd_database = ucsd_database.resized_to_scale(image_size=(128, 128),
...
def test_model():
    ...
    database = get_database(True, True)

    model.compile(optimizer=Adam(lr=1e-4), loss="mse", metrics=["mse", "mae"])

    # model.load_weights("../logs/tests/conv3d_rec_pred/{}/1551139551/weights.h5".format(mode))

    train_model(model, database, mode)
    visualize(model, database)
